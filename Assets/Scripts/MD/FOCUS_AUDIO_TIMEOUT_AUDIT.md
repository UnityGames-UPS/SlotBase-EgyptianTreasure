...
| Spin-loop force-stop entry point (low-balance error) | `SlotBehaviour.ForceStopForLowBalance()` (Egyptian Treasure — one worked example, re-derive per game, see Check 5) | any method that stops the spin-related coroutines, undoes their pre-result side effects (visual tweens, sfx, optimistic UI), resets flags/buttons to the game's own idle state, resets any sibling gamble/bonus coroutine system the same way, and shows the low-balance popup |
...
5. **`OnError(Error err)`** — low-balance vs session-expired vs generic error handling.
...
### What to look for
An error handler registered on the socket and differentiating **low-balance**, **session-expired**,
and generic errors, with WebGL-guarded messages to the JS host.

> **Why the low-balance branch exists:** balance is expected to arrive via the `balance:sync` event
> (Check 10) or a spin/gamble/bonus result payload — never as a socket `Error`. In practice, the
> backend can still surface a "balance is low"-shaped message through the generic error channel. If
> that falls through to the generic branch, the game sends `"error"`/`"session_expired"` to the JS
> host, which tears the game down and forces the player to refresh (see the platform's own
> "Launch Error / Unity Error: undefined" screen) — an unnecessarily destructive response to a
> condition the game already has a graceful UI for (the low-balance popup). The fix keeps the player
> in the game: stop the spin loop in place, reset to an idle state, and show the low-balance popup,
> without disconnecting or notifying the host.
...
Handler — the low-balance branch is checked **first**, ahead of session-expired/generic, and is
purely local (no `OnDisconnected()`, no `SendCustomMessage` to the host):
```csharp
...
### How to find everything that needs resetting — don't guess, trace it
`ForceStopForLowBalance()` (role: **spin-loop force-stop entry point**) is not a fixed template to
port verbatim — every game's spin loop is wired differently, and copying Egyptian Treasure's field
names into a game with a different structure will silently miss things. Instead, for the target game:

1. **Find the coroutine(s) that `yield return`/`WaitUntil` on the socket's "result received" flag**
   (`isResultdone` or equivalent) right after sending a spin/gamble/bonus request. This is exactly
   where execution is frozen when the backend answers with this error instead of a result — so
   everything that coroutine already did *before* that wait point is a candidate for having to be
   undone, since `StopCoroutine` alone does not unwind any of it (no `finally` runs).
2. **List every side effect the coroutine applied before that wait point**: looping/visual tweens
   (e.g. reel-spin tweens added to a list and only killed later in the normal flow), sound loops
   started for the spin, any "in-progress" UI (a spinner/sun/loading animation), and any *optimistic*
   client-side update applied before the server confirmed anything (a balance-deduction tween/text
   is the common one — revert it to the last known-good server balance, not just "whatever the label
   currently shows", since the deduction may have already visually completed).
3. **Check what the game's own existing "cancel" path already resets** — most games already have a
   button (auto-spin stop, a manual cancel) or an exhaustion path (auto-spin count hits zero) that
   returns to the same idle state you need here. Reuse/mirror that exact reset instead of inventing a
   new one — it's already been shaped to match what the rest of the UI expects (e.g. an auto-spin
   button that's disabled, not just interactable, once the count is spent — see the failure mode
   below for what happens if you only replicate part of it).
4. **Check sibling systems that share the same request/result plumbing.** If the game has a
   gamble/double-up or bonus-picking feature with its *own* coroutine(s) waiting on the same
   "result received" flag (common when they all funnel through one socket manager), the same error
   can interrupt those too — reset them symmetrically, from a method on that system, called from the
   same force-stop path.

### Reference implementation (Egyptian Treasure) — a worked example, not a template
This is what steps 1–3 above produced for *this* game specifically — `SlotBehaviour.TweenRoutine`
freezes on `WaitUntil(() => SocketManager.isResultdone)` right after sending the spin request, with
reel-spin tweens, a sun animation, a spin SFX loop, and an optimistic balance-deduction tween already
started. (Step 4 — this game's `GambleController` also has its own `WaitUntil(isResultdone)`
coroutines with no stored handles, so the same class of bug is latent there too, but resetting it
wasn't part of this pass; treat it as an open follow-up for this game, and don't skip step 4 in a
target game just because it's skipped here.) Use this to see the *shape* of a correct fix, then
re-derive the equivalent for the target game rather than reusing these field names:
```csharp
internal void ForceStopForLowBalance()
{
    ...
    // Undo the frozen coroutine's pre-result side effects — none of this unwinds on its own.
    KillAllTweens();                 // stop the looping reel-spin tweens
    ScoreTween?.Kill();
    Balance_text.text = SocketManager.PlayerData.balance.ToString("f3");  // revert the optimistic deduction
    uiManager.StopSunAnim();
    ...
    // Mirrors this game's own StopAutoSpinCoroutine end-state — ToggleButtonGrp(true) re-enables
    ...
    AutoSpinCounter = 0;
    AutoSpinNum = 0;
    AutoSpin_Text.text = AutoSpinNum.ToString();
...
- No low-balance branch (or it's checked after session-expired/generic instead of first) → a
  backend "balance is low" error falls through to the generic branch and exits the game instead of
  showing the low-balance popup in place.
- Low-balance branch calls `OnDisconnected()` or `SendCustomMessage` → unnecessarily tears down the
  session/notifies the host for a condition that doesn't require leaving the game.
- `ForceStopForLowBalance()` (or equivalent) only calls the generic `ToggleButtonGrp(true)`-style
  re-enable and skips the game's own auto-spin-exhausted button/counter reset (e.g. re-disabling
  `AutoSpin_Button`/`AutoStartMinus_Button` and zeroing the auto-spin counter) → the auto-spin button
  is left interactable pointing at a stale count instead of the idle state the player would see after
  a normal auto-spin run finishes.
- Only the coroutine itself is stopped, but not the *looping visual tweens* it started before
  freezing (e.g. reel-spin tweens added to a list and normally killed later in the success path) →
  reels are left spinning forever on screen even though the spin logically ended.
- An *optimistic* client-side update (most commonly a balance-deduction animation/text applied before
  the server confirmed the spin) is left showing the deducted value instead of being reverted to the
  last known-good server balance → the displayed balance is wrong until the next real update.
- A sibling coroutine-driven system that shares the same "wait for result" flag (a gamble/double-up
  screen, a bonus-picking screen) isn't reset the same way → that screen's loading state/cards/buttons
  are left stuck even though the spin-loop fix works correctly.
- Field/method names from Egyptian Treasure's example (`AutoSpinRoutine`, `ScoreTween`,
  `StopSpin_Button`, etc.) are copied into a different game verbatim instead of re-deriving the
  equivalent from that game's own coroutines and side effects → compiles against the wrong fields, or
  compiles but misses that game's actual frozen state entirely.
- Match string too narrow/case-sensitive (e.g. exact-match `"Balance is low"` instead of a
  lowercased `Contains("balance") && Contains("low")`) → misses backend message variants.
...
- Null `err.message` not guarded → `NullReferenceException` on `.Contains`/`.ToLower`.
...
| 5 | `OnError` (low-balance stops spin loop in place vs session-expired vs generic) | | | |
